...
**Tagline:** "Arma tu equipo de agentes IA, personalizalos y desplegalos donde quieras"
**Tipo:** Evolutivo mayor de project-genetica-platform (pivote GEN-as-a-Service a Agent Team Builder SaaS)
...
- Registro y autenticacion de usuarios (email+password, magic link, Google OAuth)
- Catalogo PUBLICO curado de agentes (23 GEN + VoltAgent), navegable sin login, solo lectura
- Creacion de proyectos con equipos de hasta 20 subagentes
- Recomendador de equipos con Claude (recibe idea, devuelve lista de agentes con justificacion)
- Validacion final obligatoria antes de activar proyecto
- Personalizacion de agentes: avatar (DALL-E o upload manual), bio estructurada, personalidad con presets especificos, estilo de comunicacion, system prompt editable
- Orquestacion: lenguaje natural + reglas simples de handoff (formulario) + agente recepcionista por default
- Conversacion entre agentes segun orquestacion definida
- Chat embebido dentro de la plataforma Gentica (testing + uso interno)
- Deploy: widget embebible + snippet HTML/JS + API publica + Slack
- Dashboard del cliente: metricas de uso, costos, historial de conversaciones, satisfaccion, evolucion temporal
- Back office Admin Streambe: gestion clientes, ingresos, costos API, suspender cuentas, ver proyectos, logs de auditoria, acceso read-only a proyectos de clientes
...
- Medicion tokens en tiempo real con alertas 80% y 100%
...
- Roles dentro del tenant: owner, editor, viewer
- API keys scoped a proyecto
- GDPR basico
...
|-------|-------------|----------|
| **Visitante** | Usuario no registrado | Ver landing, pricing, catalogo publico, registrarse |
| **Cliente Free** | Usuario registrado tier gratuito | 1 proyecto activo (pausar para cambiar), 100K tokens/mes, personalizar agentes, deploy en widget + API (sin Slack) |
| **Cliente Pro** | Usuario registrado tier pago | 5 proyectos activos, 2M tokens incluidos + overage, todas las funcionalidades MVP incluyendo Slack |
| **Cliente Enterprise** | Empresa con contrato custom | Limites custom, soporte dedicado, contacto comercial via form a CRM + email a ventas Streambe |
| **Admin Streambe** | Administrador interno de Streambe | Gestion de clientes, ingresos, control costos API, suspender cuentas, ver todos los proyectos (read-only), logs de auditoria, gestionar catalogo de agentes |
| **Owner (tenant)** | Creador del proyecto dentro de un tenant | Control total del proyecto, gestionar miembros |
| **Editor (tenant)** | Miembro del tenant con permisos de edicion | Editar agentes, orquestacion, deploy |
| **Viewer (tenant)** | Miembro del tenant con permisos de lectura | Ver proyecto, conversaciones, metricas. Sin edicion |
...
|----|-------|-------------|
| EPIC-01 | Autenticacion y Onboarding | Registro (email+password, magic link, Google OAuth), login, recuperacion de password, wizard de onboarding |
| EPIC-02 | Catalogo de Agentes | Catalogo PUBLICO navegable, detalle de cada agente, filtros por skill/categoria/origen, busqueda por texto libre y por tag |
| EPIC-03 | Gestion de Proyectos | Crear, editar, pausar, eliminar proyectos. Flujo: describir idea, IA sugiere equipo, usuario ajusta, valida, crea. Validacion final obligatoria |
| EPIC-04 | Armado de Equipos | Seleccionar agentes del catalogo, agregar/quitar del equipo, limite de 20, recomendador IA |
| EPIC-05 | Personalizacion de Agentes | Avatar (DALL-E o upload), bio estructurada, personalidad con presets especificos, estados emocionales, estilo comunicacion, system prompt editable |
| EPIC-06 | Orquestacion | Definir flujo de trabajo entre agentes via lenguaje natural y reglas de handoff, agente recepcionista por default |
| EPIC-07 | Base de Conocimiento | Upload de documentos y archivos que los agentes pueden consultar |
| EPIC-08 | Deploy y Canales | Chat embebido en plataforma, widget embebible, snippet HTML/JS, API publica, integracion Slack |
| EPIC-09 | Conversacion y Runtime | Motor de ejecucion multi-agente, conversacion entre agentes, interaccion usuario-agentes |
| EPIC-10 | Dashboard del Cliente | Metricas de uso, costos, historial de conversaciones, satisfaccion, evolucion temporal, tope de gasto |
| EPIC-11 | Billing y Suscripciones | Stripe, tiers, tokens, overage, invoices, tope de gasto, medicion tokens en tiempo real |
| EPIC-12 | Back Office Admin | Panel de administracion Streambe con metricas de negocio, logs de auditoria, acceso read-only a proyectos |
| EPIC-13 | Seguridad y Aislamiento | RLS, schemas por proyecto, GDPR basico, roles tenant (owner/editor/viewer), API keys scoped |
...
| RF-004 | El sistema debe permitir login con Google OAuth | Must |
...
| RF-088 | El sistema debe permitir login con magic link (enlace enviado por email, sin password) | Must |

### EPIC-02 — Catalogo de Agentes

| ID | Requerimiento | Prioridad |
|----|---------------|-----------|
| RF-008 | El sistema debe mostrar un catalogo PUBLICO de agentes curado por Streambe (23 agentes GEN + agentes VoltAgent), navegable sin necesidad de login | Must |
| RF-009 | Cada agente del catalogo debe mostrar: nombre, rol, descripcion, skills, CV, bio, avatar por defecto | Must |
| RF-010 | El catalogo debe permitir filtrar agentes por skill, categoria y origen | Must |
...
| RF-089 | El catalogo debe permitir busqueda por texto libre Y por tag | Must |
...
| RF-014 | La creacion de un proyecto debe seguir el flujo: usuario describe su idea, la IA sugiere un equipo de agentes, el usuario ajusta, valida y crea | Must |
| RF-015 | Cliente Free: maximo 1 proyecto activo. Para crear otro debe pausar (stop) el actual (flujo explicito: pausar activo, activar otro) | Must |
...
| RF-090 | El sistema debe ofrecer un recomendador de equipos con Claude: recibe la descripcion de la idea del usuario y devuelve una lista de agentes sugeridos con justificacion para cada uno | Must |
| RF-091 | El sistema debe requerir una validacion final OBLIGATORIA antes de activar un proyecto (el usuario confirma equipo, orquestacion y configuracion antes de que el proyecto quede activo) | Must |
...
| ID | Requerimiento | Prioridad |
|----|---------------|-----------|
| RF-027 | El usuario debe poder generar un avatar personalizado describiendo en texto como quiere que se vea (generacion via DALL-E) O subir una imagen manualmente (upload) | Must |
| RF-028 | El usuario debe poder regenerar el avatar si no le gusta el resultado (DALL-E) o subir una nueva imagen (upload) | Must |
| RF-029 | El usuario debe poder escribir/editar la bio del agente con campos estructurados (rol, experiencia, expertise) ademas de texto libre | Must |
...
| RF-031 | El usuario debe poder seleccionar un preset de personalidad para el agente de la siguiente lista: calmado, entusiasta, formal, creativo, analitico, empatico, directo | Must |
...
| RF-092 | El usuario avanzado debe poder editar el system prompt del agente directamente, con un warning visible indicando que cambios incorrectos pueden afectar el comportamiento del agente | Should |
...
| RF-093 | Todo proyecto debe tener por default un agente "recepcionista" que distribuye las consultas entrantes al agente correcto del equipo segun la orquestacion | Must |
...
| ID | Requerimiento | Prioridad |
|----|---------------|-----------|
| RF-094 | El sistema debe proveer un chat embebido DENTRO de la plataforma Gentica para testing y uso interno del proyecto (no requiere deploy externo) | Must |
...
| RF-049 | La API debe requerir autenticacion via API key scoped a proyecto, generada por el usuario | Must |
| RF-050 | El sistema debe permitir conectar un proyecto con un workspace de Slack (cuenta Streambe) para interactuar con los agentes via Slack. Canal Slack disponible solo para tiers Pro y Enterprise | Must |
...
| RF-054 | El usuario debe poder iniciar una conversacion con el equipo de agentes de su proyecto desde la plataforma (chat embebido) | Must |
| RF-055 | El sistema debe enrutar mensajes del usuario al agente correspondiente segun la orquestacion (via recepcionista por default) | Must |
...
| RF-061 | El dashboard debe mostrar el costo acumulado del periodo actual y la proyeccion al cierre del mes, en USD y ARS | Must |
...
| RF-065 | El dashboard debe mostrar alertas cuando el consumo alcanza el 80% y 100% del limite | Must |
| RF-095 | El dashboard debe mostrar conversaciones totales, conversaciones unicas y desglose por canal | Must |
| RF-096 | El dashboard debe mostrar tokens consumidos con costo estimado en USD y ARS | Must |
| RF-097 | El dashboard debe mostrar agentes mas usados y tasa de handoff entre agentes | Should |
| RF-098 | El dashboard debe permitir feedback de satisfaccion por conversacion (thumbs up/down opcional por el usuario final) | Should |
| RF-099 | El dashboard debe mostrar evolucion temporal de metricas en periodos de 7 dias, 30 dias y 90 dias | Must |
...
| RF-067 | Tier Free: 1 proyecto activo, 100K tokens/mes, canales limitados a widget + API (sin Slack). Al agotar tokens, se bloquea el uso hasta el proximo periodo. Sin cobro | Must |
| RF-068 | Tier Pro: USD 49/mes, 5 proyectos activos, 2M tokens incluidos. Overage: USD 15 por millon adicional. Todos los canales habilitados | Must |
| RF-069 | Tier Enterprise: pricing custom. Formulario de contacto que envia los datos al CRM de Streambe y dispara un email a ventas Streambe | Must |
...
| RF-100 | El sistema debe medir tokens consumidos en tiempo real y mostrar alertas al 80% y 100% del limite del tier | Must |
| RF-101 | Para clientes Free: para cambiar de proyecto activo, el flujo explicito es pausar el proyecto activo actual y luego activar el otro proyecto | Must |
...
| RF-077 | El Admin debe poder ver el dashboard de ingresos: MRR, ARR, churn rate, LTV | Must |
...
| RF-102 | El Admin debe ver clientes activos desglosados por plan | Must |
| RF-103 | El Admin debe ver consumo agregado de tokens y margenes | Must |
| RF-104 | El Admin debe ver ranking de top clientes por revenue | Should |
| RF-105 | El Admin debe ver funnel de conversion Free a Pro | Should |
| RF-106 | El panel Admin debe tener logs de auditoria (quién hizo qué, cuándo) | Must |
| RF-107 | El Admin debe tener acceso read-only a los proyectos de cualquier cliente (para soporte) | Must |
...
| RF-108 | El sistema debe soportar roles dentro del tenant: owner (control total), editor (editar proyecto), viewer (solo lectura) | Must |
| RF-109 | Las API keys deben estar scoped a proyecto — una API key solo da acceso al proyecto para el que fue generada | Must |
...
|----|-----------|---------------|
| RNF-001 | Escalabilidad | Vercel + Supabase serverless en MVP |
| RNF-002 | Escalabilidad | ADR documentando plan K8s cuando MRR > USD X (threshold a definir) |
| RNF-003 | Escalabilidad | La arquitectura debe permitir escalar horizontalmente sin reescritura |
| RNF-004 | Aislamiento | Supabase RLS por project_id |
| RNF-005 | Aislamiento | API keys scoped por proyecto |
| RNF-006 | Aislamiento | Secretos por proyecto en vault (no env vars globales) |
| RNF-007 | Performance | Primera respuesta de agente < 3 segundos |
| RNF-008 | Performance | Dashboard < 1.5s FCP |
| RNF-009 | Performance | Generacion de avatar DALL-E < 15 segundos |
| RNF-010 | Disponibilidad | 99.5% uptime MVP (excluyendo mantenimiento programado) |
| RNF-011 | Disponibilidad | 99.9% post-MVP (documentar como objetivo evolutivo) |
| RNF-012 | Seguridad | OWASP Top 10 compliance |
| RNF-013 | Seguridad | Content Security Policy (CSP) headers |
| RNF-014 | Seguridad | Rate limiting por API key (no solo por endpoint) |
| RNF-015 | Seguridad | Audit logs de acciones administrativas y sensibles |
| RNF-016 | Seguridad | Tokens y API keys nunca en texto plano |
| RNF-017 | Seguridad | HTTPS/TLS en todas las comunicaciones |
| RNF-018 | Compliance | GDPR basico: export y delete de datos del usuario a pedido |
| RNF-019 | Compliance | SOC2/HIPAA: evolutivo, no MVP |
| RNF-020 | Observabilidad | Logs estructurados (JSON) |
| RNF-021 | Observabilidad | Sentry para error tracking frontend + backend |
| RNF-022 | Observabilidad | Metricas de tokens en tiempo real |
| RNF-023 | Observabilidad | Health checks en todos los servicios |
| RNF-024 | Backups | Backups diarios automaticos de Supabase |
| RNF-025 | Backups | Retencion 30 dias |
| RNF-026 | Usabilidad | Chrome, Firefox, Safari, Edge ultimas 2 versiones |
| RNF-027 | Usabilidad | Responsive: desktop y tablet. Mobile funcional no optimizado |
| RNF-028 | Usabilidad | Idioma espanol MVP, ingles fase 2 |
| RNF-029 | Mantenibilidad | Cobertura tests 80% logica de negocio critica |
| RNF-030 | Mantenibilidad | Documentacion API publica con OpenAPI/Swagger |
...
#### US-016: Login con magic link

```
Como usuario registrado,
quiero iniciar sesion con un enlace magico enviado a mi email,
para acceder sin necesidad de recordar un password.
```

```gherkin
Escenario: Login con magic link exitoso
  Dado que estoy en la pagina de login
  Cuando selecciono "Iniciar sesion con magic link"
  Y ingreso mi email registrado
  Entonces recibo un email con un enlace de acceso
  Y al hacer click en el enlace soy redirigido a mi dashboard

Escenario: Magic link con email no registrado
  Dado que ingreso un email que no esta registrado
  Cuando solicito un magic link
  Entonces veo un mensaje "No encontramos una cuenta con ese email"
```

#### US-017: Login con Google OAuth

```
Como visitante o usuario registrado,
quiero iniciar sesion con mi cuenta de Google,
para acceder rapidamente sin crear un password.
```
...
Escenario: Login con Google exitoso
  ...
  Entonces soy redirigido a mi dashboard
  Y si es mi primera vez se crea mi cuenta automaticamente
```
...
#### US-018: Navegar catalogo publico

```
Como visitante (sin login),
quiero explorar el catalogo de agentes disponibles,
para entender que ofrece la plataforma antes de registrarme.
```

```gherkin
Escenario: Navegar catalogo sin login
  Dado que soy un visitante no registrado
  Cuando accedo a la pagina del catalogo
  Entonces veo la lista de agentes con nombre, rol, descripcion, skills y avatar
  Y puedo filtrar por skill, categoria y origen
  Y puedo buscar por texto libre o por tag
  Y no necesito registrarme para ver el catalogo

Escenario: Ver detalle de agente
  Dado que estoy en el catalogo
  Cuando hago click en un agente
  Entonces veo su detalle completo: nombre, rol, descripcion, skills, CV, bio, avatar
```

### EPIC-03 — Gestion de Proyectos

#### US-004: Crear proyecto con recomendador IA

```
Como cliente,
quiero crear un nuevo proyecto describiendo lo que necesito,
para que la IA me sugiera un equipo de agentes adecuado con justificacion.
```

```gherkin
Escenario: Creacion conversacional de proyecto con recomendador
  ...
  Y basado en mi respuesta, Claude sugiere un nombre, descripcion y equipo de agentes con justificacion para cada agente seleccionado
  Y puedo ajustar el equipo sugerido (agregar/quitar agentes)
  Y debo confirmar la validacion final antes de que el proyecto se active
...
#### US-019: Validacion final antes de activar proyecto

```
Como cliente,
quiero revisar y confirmar todo antes de activar mi proyecto,
para asegurarme de que la configuracion es correcta.
```

```gherkin
Escenario: Validacion final exitosa
  Dado que termine de configurar mi proyecto (equipo, orquestacion, personalizacion)
  Cuando llego al paso de validacion final
  Entonces veo un resumen de: equipo de agentes, orquestacion definida, canales de deploy
  Y debo hacer click en "Activar proyecto" para confirmar
  Y solo despues de confirmar el proyecto pasa a estado "Activo"

Escenario: Volver atras desde validacion
  Dado que estoy en el paso de validacion final
  Cuando detecto algo que quiero cambiar
  Entonces puedo volver a pasos anteriores sin perder la configuracion
```
...
Escenario: Cambiar proyecto activo (Free)
  Dado que soy cliente Free con un proyecto activo y otro pausado
  Cuando quiero trabajar con el proyecto pausado
  Entonces primero debo pausar el proyecto activo
  Y luego puedo activar el otro proyecto
```
...
#### US-007: Generar avatar con DALL-E o upload manual

```
Como cliente,
quiero darle una identidad visual a mi agente generando un avatar con IA o subiendo una imagen,
para que tenga una identidad visual unica.
```

```gherkin
Escenario: Generacion exitosa de avatar con DALL-E
...
Escenario: Upload manual de avatar
  Dado que estoy editando un agente de mi proyecto
  Cuando selecciono "Subir imagen"
  Y subo un archivo PNG o JPG
  Entonces la imagen se muestra como preview
  Y puedo confirmarla como avatar del agente
...
Escenario: Seleccionar preset de personalidad
  ...
  Cuando selecciono un preset de la lista: calmado, entusiasta, formal, creativo, analitico, empatico o directo
...
#### US-020: Editar bio estructurada

```
Como cliente,
quiero definir la bio de mi agente con campos estructurados,
para que tenga un perfil profesional completo.
```

```gherkin
Escenario: Editar bio con campos estructurados
  Dado que estoy editando un agente
  Cuando completo los campos: rol, experiencia, expertise
  Y opcionalmente agrego texto libre adicional
  Entonces la bio se guarda con la estructura completa
  Y se muestra en el perfil del agente
```

#### US-021: Editar system prompt (avanzado)

```
Como usuario avanzado,
quiero editar el system prompt de mi agente directamente,
para tener control total sobre su comportamiento.
```

```gherkin
Escenario: Editar system prompt con warning
  Dado que estoy editando un agente
  Cuando accedo a la opcion "System prompt" (seccion avanzada)
  Entonces veo un warning: "Cambios incorrectos pueden afectar el comportamiento del agente. Procede con cuidado"
  Y puedo editar el system prompt libremente
  Y al guardar, los cambios se aplican inmediatamente al agente
```
...
Escenario: Agente recepcionista por default
  Dado que mi proyecto tiene al menos 2 agentes
  Cuando un usuario externo envia un mensaje
  Entonces el agente recepcionista (presente por default) recibe la consulta
  Y la distribuye al agente correcto segun la orquestacion definida
```

### EPIC-08 — Deploy y Canales

#### US-022: Chat embebido en la plataforma

```
Como cliente,
quiero conversar con mis agentes directamente dentro de Gentica,
para testear y usar mi equipo sin necesidad de deploy externo.
```

```gherkin
Escenario: Usar chat embebido
  Dado que mi proyecto tiene agentes configurados y orquestacion definida
  Cuando abro la interfaz de chat embebido del proyecto en Gentica
  Entonces puedo conversar con mis agentes en tiempo real
  Y el chat funciona igual que en cualquier canal de deploy externo
```
...
Escenario: Conexion exitosa con Slack
  Dado que soy cliente Pro o Enterprise
  Y estoy en la seccion "Deploy" de mi proyecto
...
Escenario: Slack no disponible en Free
  Dado que soy cliente Free
  Cuando veo las opciones de deploy
  Entonces el canal Slack aparece bloqueado con un mensaje "Disponible en Pro y Enterprise"
```
...
Escenario: Iniciar conversacion
  ...
  Entonces el recepcionista enruta mi mensaje al agente correspondiente segun la orquestacion
...
```
Como cliente,
quiero ver metricas completas de uso, costos y satisfaccion,
para controlar mis gastos y evaluar el rendimiento de mis agentes.
```

```gherkin
Escenario: Dashboard con metricas completas
  Dado que estoy logueado y tengo al menos un proyecto
  Cuando accedo a mi dashboard
  Entonces veo conversaciones totales, unicas y desglose por canal
  Y veo tokens consumidos con costo estimado en USD y ARS
  Y veo agentes mas usados y tasa de handoff
  Y veo el estado de mi suscripcion (tier, tokens incluidos, consumidos)
  Y veo alertas si estoy por encima del 80% de mi limite
  Y puedo seleccionar periodo: 7 dias, 30 dias o 90 dias

Escenario: Ver satisfaccion
  Dado que mis usuarios finales dejaron feedback (thumbs up/down)
  Cuando accedo a la seccion de satisfaccion del dashboard
  Entonces veo el porcentaje de satisfaccion por proyecto y por agente
...
#### US-023: Contacto Enterprise

```
Como empresa interesada en el plan Enterprise,
quiero completar un formulario de contacto,
para que el equipo comercial de Streambe me contacte.
```

```gherkin
Escenario: Enviar formulario Enterprise
  Dado que estoy en la pagina de pricing
  Cuando hago click en "Contactar ventas" del plan Enterprise
  Y completo el formulario con nombre, email, empresa y necesidades
  Entonces los datos se envian al CRM de Streambe
  Y se dispara un email a ventas de Streambe
  Y veo un mensaje "Nuestro equipo te contactara en menos de 24hs"
```
...
```
Como Admin Streambe,
quiero gestionar los clientes y ver metricas de negocio completas,
...
Escenario: Ver metricas de negocio completas
  Dado que estoy en el dashboard Admin
  Cuando accedo a la seccion "Ingresos"
  Entonces veo MRR, ARR, churn rate, LTV
  Y veo clientes activos desglosados por plan
  Y veo consumo agregado de tokens y margenes
  Y veo ranking de top clientes por revenue
  Y veo funnel de conversion Free a Pro

Escenario: Acceso read-only a proyecto de cliente
  Dado que estoy en el detalle de un cliente
  Cuando hago click en uno de sus proyectos
  Entonces puedo ver la configuracion, conversaciones y metricas del proyecto
  Y no puedo editar nada (solo lectura para soporte)

Escenario: Ver logs de auditoria
  Dado que estoy en el panel Admin
  Cuando accedo a la seccion "Auditoria"
  Entonces veo un log de acciones: quien hizo que, cuando, sobre que recurso
  Y puedo filtrar por usuario, tipo de accion y fecha
```

### EPIC-13 — Seguridad y Aislamiento

#### US-024: Roles dentro del tenant

```
Como owner de un proyecto,
quiero asignar roles a miembros de mi equipo,
para controlar quien puede ver y editar mi proyecto.
```

```gherkin
Escenario: Invitar miembro como editor
  Dado que soy owner de un proyecto
  Cuando invito a un usuario por email y le asigno el rol "editor"
  Entonces el usuario recibe una invitacion por email
  Y al aceptar puede editar agentes, orquestacion y deploy del proyecto
  Y no puede eliminar el proyecto ni cambiar roles

Escenario: Asignar rol viewer
  Dado que soy owner de un proyecto
  Cuando invito a un usuario como "viewer"
  Entonces el usuario puede ver el proyecto, conversaciones y metricas
  Y no puede editar nada
```
...
|---------|------|-----------|---------|
| **Anthropic Claude** | API | Modelo IA para respuestas de agentes + recomendador de equipos | API de chat/completions con streaming. Contabilizacion de tokens in/out por request |
...
| **Supabase** | BaaS | Auth, DB, Storage, RLS | Autenticacion (email + magic link + Google OAuth), Postgres con RLS, Storage para documentos/avatares |
| **Slack** | API (@slack/bolt) | Canal de deploy (Pro/Enterprise) | Bot que conecta agentes del proyecto a canales de Slack. Cuenta Streambe |
...
| **CRM Streambe** | API/Webhook | Contacto Enterprise | Formulario Enterprise envia datos al CRM y dispara email a ventas |
...
[Visitante llega a landing]
 ...
 [Crear cuenta]
    |
    v
 [Email de verificacion (si email+pass)]
    |
    v
 [Primer login]
    |
...
### 9.2 Crear Proyecto (con recomendador IA)
...
[Dashboard del cliente]
 ...
 [Agente guia pregunta
  que quiere construir]
    |
    v
 [Usuario describe idea]
    |
    v
 [Claude recomienda equipo
  con justificacion por agente]
    |
    v
 [Usuario ajusta equipo
  (agregar/quitar agentes)]
    |
    v
 [VALIDACION FINAL OBLIGATORIA]
 [Resumen: equipo, orquestacion,
  canales — usuario confirma]
    |
    v
 [Proyecto ACTIVO con
  copias independientes
  de agentes + recepcionista]
```
 ...
 [Seleccionar v
  agente]   [Seleccionar equipo]
   ...
   [Para cada agente:]
    +----+----+----+----+
    |    |    |    |    |
  avatar bio pers estilo prompt
    |    |    |    |    |
    v    v    v    v    v
  [DALL-E [estruc-[preset][config][system
   o      turada]               prompt]
   upload]
         |
         v
...
[Proyecto con equipo y orquestacion]
   ...
   [Seccion "Deploy"]
    ...
    +----+----+----+----+----+
    |    |    |    |         |
  Chat  Widget Snippet API  Slack
  embed  |    |    |       (Pro+)
    |    v    v    v         |
    v  [Config [Copiar [Generar  v
 [Listo] color/  HTML/   API   [Autorizar
         pos]    JS]    key]   workspace]
         |    |    |         |
         v    v    v         v
       [Preview][Listo] [Docs  [Bot activo
         |              API]   en canal]
         v
       [Copiar snippet]
```
...
| Magic link | Si | -- |
| Google OAuth | Si | -- |
| Catalogo PUBLICO de agentes curado | Si | -- |
| Catalogo: filtros por skill, categoria, origen | Si | -- |
| Catalogo: busqueda por texto libre y tag | Si | -- |
| Creacion de agentes desde cero | No | Si |
| Marketplace de equipos | No | Si |
| Proyectos (crear, pausar, eliminar) | Si | -- |
| Recomendador de equipos con Claude | Si | -- |
| Validacion final obligatoria | Si | -- |
| Equipo de hasta 20 subagentes | Si | -- |
| Equipos pre-armados | Si | -- |
| Avatar DALL-E + upload manual | Si | -- |
| Bio estructurada (rol, experiencia, expertise) | Si | -- |
| CV + personalidad + estados | Si | -- |
| Presets: calmado/entusiasta/formal/creativo/analitico/empatico/directo | Si | -- |
| System prompt editable (avanzado) | Si | -- |
| Estilo de comunicacion | Si | -- |
| Orquestacion lenguaje natural | Si | -- |
| Orquestacion formulario de reglas | Si | -- |
| Agente recepcionista por default | Si | -- |
| Orquestacion editor visual flowchart | No | Si |
| Base de conocimiento (upload docs) | Si | -- |
| Chat embebido en plataforma | Si | -- |
| Widget embebible | Si | -- |
| Snippet HTML/JS | Si | -- |
| API publica REST | Si | -- |
| Slack (solo Pro/Enterprise) | Si | -- |
| WhatsApp Business API | No | Si |
| Telegram | No | Si |
| Runtime multi-agente | Si | -- |
| Conversacion inter-agente | Si | -- |
| Dashboard cliente: conversaciones, tokens, costos USD/ARS | Si | -- |
| Dashboard cliente: agentes mas usados, tasa handoff | Si | -- |
| Dashboard cliente: satisfaccion (thumbs up/down) | Si | -- |
| Dashboard cliente: evolucion temporal 7d/30d/90d | Si | -- |
| Historial de conversaciones | Si | -- |
| Tope de gasto configurable | Si | -- |
| Medicion tokens en tiempo real con alertas 80%/100% | Si | -- |
| Stripe billing multi-moneda | Si | -- |
| Tiers Free / Pro / Enterprise | Si | -- |
| Free: canales limitados (widget + API) | Si | -- |
| Enterprise: form a CRM + email ventas | Si | -- |
| Invoices automaticos | Si | -- |
| Back office Admin Streambe | Si | -- |
| Back office: MRR, ARR, churn, LTV | Si | -- |
| Back office: funnel Free a Pro | Si | -- |
| Back office: logs de auditoria | Si | -- |
| Back office: read-only proyectos cliente | Si | -- |
| Back office roles Soporte y Finanzas | No | Si |
| RLS + schemas por proyecto | Si | -- |
| Roles tenant: owner/editor/viewer | Si | -- |
| API keys scoped a proyecto | Si | -- |
| GDPR basico | Si | -- |
| SOC2 / HIPAA | No | Si |
| Idioma espanol | Si | -- |
| Idioma ingles | No | Si |
...
| **Catalogo** | Coleccion curada de agentes disponibles, gestionada por Streambe. Publico (visible sin login) |
...
| **Recepcionista** | Agente por default que recibe consultas entrantes y las distribuye al agente correcto |
| **Widget** | Componente web embebible para interactuar con los agentes |
| **Chat embebido** | Interfaz de chat dentro de la plataforma Gentica (no requiere deploy externo) |
...
| **Tenant** | Espacio aislado del proyecto con roles: owner, editor, viewer |
| **Magic link** | Enlace de autenticacion enviado por email, sin necesidad de password |
| **System prompt** | Instrucciones base que definen el comportamiento del agente (editable por usuarios avanzados) |
...
8. Streambe cuenta con un CRM operativo para recibir leads del formulario Enterprise

## 13. Dependencias

| Dependencia | Impacto |
|-------------|---------|
| API Anthropic Claude disponible | Sin ella no hay runtime de agentes ni recomendador de equipos |
| API OpenAI DALL-E disponible | Sin ella no hay generacion de avatares (feature degradable — queda upload manual) |
...
| CRM Streambe configurado | Sin el no se reciben leads Enterprise (degradable a email) |
...
4. ~~Flujo exacto de Enterprise: formulario de contacto llega a donde?~~ **RESUELTA:** Form envia datos al CRM de Streambe + dispara email a ventas Streambe.
...
*v2.0 — 2026-04-09 — Incorpora pivote a Agent Team Builder SaaS, catalogo publico, recomendador IA, chat embebido, roles tenant, presets especificos, system prompt editable, agente recepcionista, metricas expandidas, back office con auditoria, Enterprise a CRM*
